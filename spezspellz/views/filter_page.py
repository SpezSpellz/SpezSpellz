...
class FilterPage(View):
    ...
    def get(self, request):
        ...
        if sort_option == "recent":
            ...
            spells = spells.annotate(comment_count=Count('spellcomment')).order_by('-comment_count')
        elif sort_option == "least_commented":
            spells = spells.annotate(comment_count=Count('spellcomment')).order_by('comment_count')
        elif sort_option == "most_viewed":
            spells = spells.annotate(view_count=Count('spellhistoryentry')).order_by('-view_count')
        elif sort_option == "least_viewed":
            spells = spells.annotate(view_count=Count('spellhistoryentry')).order_by('view_count')
        elif sort_option == "most_rated":
            spells = spells.annotate(average_rating=Avg('review__star')).order_by('-average_rating')
